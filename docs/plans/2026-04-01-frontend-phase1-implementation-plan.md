...
export function StatusBar() {
  const { activeModel, tokensPerSec, vramUsed, vramTotal, trainingStatus, federationPeers } = useSystemStore()

  return (
    <footer className="h-[28px] flex items-center px-3 gap-4 bg-[var(--color-bg-surface-1)] border-t border-[var(--color-border-subtle)] text-[11px] text-[var(--color-text-secondary)] flex-shrink-0">
      ...
      {vramUsed !== null && vramTotal !== null && (
        <>
          <span className="text-[var(--color-border-default)]">|</span>
          <span>GPU {vramUsed.toFixed(1)}/{vramTotal} GB</span>
        </>
      )}
      ...
      {/* Training */}
      <span className="text-[var(--color-border-default)]">|</span>
      <span className={trainingStatus === 'running' ? 'text-[var(--color-yellow-400)]' : ''}>
        Training: {trainingStatus === 'idle' ? 'Idle' : trainingStatus === 'running' ? 'Running' : 'Complete'}
      </span>

      {/* Federation */}
      <span className="text-[var(--color-border-default)]">|</span>
      <span className={federationPeers > 0 ? 'text-[var(--color-green-400)]' : ''}>
        Fed: {federationPeers > 0 ? `${federationPeers} peers` : 'Offline'}
      </span>
    </footer>
  )
}
...
test('Dashboard shows system health cards', () => {
  render(<Dashboard />)
...
function HealthCard({ title, status, value, colour }: {
  title: string, status: string, value: string | null, colour: 'green' | 'yellow' | 'red'
}) {
  const dot = { green: 'bg-[var(--color-green-500)]', yellow: 'bg-[var(--color-yellow-400)]', red: 'bg-[var(--color-red-400)]' }[colour]
  return (
    <div className="flex-1 bg-[var(--color-bg-surface-2)] rounded-lg border border-[var(--color-border-subtle)] p-4">
      <div className="flex items-center gap-2 mb-2">
        <span className={`w-2 h-2 rounded-full ${dot}`} />
        <span className="text-sm font-medium text-[var(--color-text-primary)]">{title}</span>
      </div>
      <p className="text-sm text-[var(--color-text-secondary)]">{status}</p>
      {value && <p className="text-xs text-[var(--color-text-muted)] mt-1">{value}</p>}
    </div>
  )
}
...
export function Dashboard() {
  ...
  const inferenceColour = ollamaOnline ? 'green' : 'red'
  const gpuColour = gpuTemp != null ? (gpuTemp > 80 ? 'yellow' : 'green') : 'red'
  const trainingColour = trainingStatus === 'running' ? 'yellow' : 'green'

  return (
    <div data-testid="screen-dashboard" className="p-6 max-w-5xl">
      <h1 className="text-xl font-semibold text-[var(--color-text-primary)] mb-6">Dashboard</h1>

      {/* System Health */}
      <section className="mb-6">
        <h2 className="text-sm font-medium text-[var(--color-text-secondary)] uppercase tracking-wider mb-3">System Health</h2>
        <div className="flex gap-4">
          <HealthCard
            title="Inference"
            status={ollamaOnline ? 'Ready' : 'Offline'}
            value={tokensPerSec != null ? `${tokensPerSec.toFixed(0)} tok/s` : null}
            colour={inferenceColour}
          />
          <HealthCard
            title="GPU Health"
            status={gpuTemp != null ? `${gpuTemp}°C` : 'Unknown'}
            value={vramUsed != null && vramTotal != null ? `${vramUsed.toFixed(1)}/${vramTotal} GB VRAM` : null}
            colour={gpuColour}
          />
          <HealthCard
            title="Training"
            status={trainingStatus === 'idle' ? 'Idle' : trainingStatus === 'running' ? 'Running' : 'Complete'}
            value={null}
      ...
      {/* Active Model */}
      <section className="mb-6">
        <h2 className="text-sm font-medium text-[var(--color-text-secondary)] uppercase tracking-wider mb-3">Active Model</h2>
        <div className="bg-[var(--color-bg-surface-2)] rounded-lg border border-[var(--color-border-subtle)] p-4">
          {activeModel ? (
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-[var(--color-text-primary)]">{activeModel}</p>
                {tokensPerSec != null && (
                  <p className="text-xs text-[var(--color-text-secondary)] mt-1">{tokensPerSec.toFixed(1)} tok/s</p>
                )}
              </div>
              <button
                onClick={() => setActive('models')}
                className="text-xs text-[var(--color-accent-400)] hover:text-[var(--color-accent-500)] transition-colors"
              >
                Switch Model →
              </button>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => setActive('models')}
                className="text-xs px-3 py-1.5 bg-[var(--color-accent-500)] text-white rounded-md hover:bg-[var(--color-accent-600)] transition-colors"
              >
                Browse Models
              </button>
            </div>
      ...
      {/* Quick Actions */}
      <section>
        <h2 className="text-sm font-medium text-[var(--color-text-secondary)] uppercase tracking-wider mb-3">Quick Actions</h2>
        <div className="flex gap-3">
          {[
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-[var(--color-bg-surface-2)] border border-[var(--color-border-default)] rounded-lg text-sm text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] hover:border-[var(--color-border-strong)] transition-colors"
            >
              <Icon size={14} />
              {label}
            </button>
...
**Step 4: Implement Models screen (sidebar + detail panel)**

See design: `docs/plans/2026-04-01-ui-ux-design.md` Section 4.2.

Implement a two-column layout:
- Left: INSTALLED list with model names, active indicator dot
- Right: Detail panel with model info, performance placeholders, action buttons (Set Active, Fine-tune, Delete)

The full implementation should use `useEffect` to call `useModelsStore.getState().loadInstalled()` on mount.
